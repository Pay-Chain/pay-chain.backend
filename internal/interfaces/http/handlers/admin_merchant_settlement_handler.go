...
type AdminMerchantSettlementHandler struct {
...
func NewAdminMerchantSettlementHandler(
	merchantRepo repositories.MerchantRepository,
	settlementProfileRepo repositories.MerchantSettlementProfileRepository,
	chainRepo repositories.ChainRepository,
	tokenRepo repositories.TokenRepository,
) *AdminMerchantSettlementHandler {
	return &AdminMerchantSettlementHandler{
...
func (h *AdminMerchantSettlementHandler) GetSettlementProfile(c *gin.Context) {
	merchantID, ok := parseMerchantIDParam(c)
	if !ok {
		return
	}
	if _, err := h.merchantRepo.GetByID(c.Request.Context(), merchantID); err != nil {
	...
	profile, err := h.settlementProfileRepo.GetByMerchantID(c.Request.Context(), merchantID)
	if err != nil {
		if err == domainerrors.ErrNotFound {
...
func (h *AdminMerchantSettlementHandler) UpsertSettlementProfile(c *gin.Context) {
	merchantID, ok := parseMerchantIDParam(c)
	if !ok {
		return
	}
	...
	existing, err := h.settlementProfileRepo.GetByMerchantID(c.Request.Context(), merchantID)
...
func parseMerchantIDParam(c *gin.Context) (uuid.UUID, bool) {
...
func resolveChainForSettlement(ctx context.Context, chainRepo repositories.ChainRepository, raw string) (*entities.Chain, error) {
	normalized := strings.TrimSpace(raw)
	if normalized == "" {
		return nil, domainerrors.BadRequest("dest_chain is required")
	}
	if strings.Contains(normalized, ":") {
		chain, err := chainRepo.GetByCAIP2(ctx, normalized)
		if err == nil && chain != nil {
			return chain, nil
		}
		return nil, domainerrors.BadRequest("dest_chain is not supported")
	}
	chain, err := chainRepo.GetByChainID(ctx, normalized)
	if err != nil || chain == nil {
		return nil, domainerrors.BadRequest("dest_chain is not supported")
	}
	return chain, nil
